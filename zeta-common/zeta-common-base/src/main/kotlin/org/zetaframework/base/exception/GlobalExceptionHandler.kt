package org.zetaframework.base.exception
...
class GlobalExceptionHandler {
     ...
     *
     * 说明：
     * 用于校验接口参数（非Json）失败，抛出的异常处理。
     *
     * 例如：
     * ```
     * @Validated
     * class DemoController {
     *     fun getData(@RequestParam @NotNull(message ="数据不能为空") data: Int) {
     *          ...
     *     }
     * }
     * ```
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    fun constraintViolationException(ex: ConstraintViolationException): ApiResult<*>? {
        logger.warn("ConstraintViolationException:", ex)
        val violations = ex.constraintViolations
        val message = violations.joinToString(";") { obj: ConstraintViolation<*> -> obj.message }
        return ApiResult.result(ErrorCodeEnum.ERR_ARGUMENT_TYPE_MISMATCH_EXCEPTION.code, message, ex.message)
    }
